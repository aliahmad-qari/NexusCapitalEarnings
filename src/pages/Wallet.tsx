...
import { Download, Upload, Wallet as WalletIcon, ChevronRight, Activity, Award, ArrowUpRight, ArrowDownRight, History as HistoryIcon, Clock, CheckCircle2, AlertCircle, Repeat, Lock, X, Shield, Cpu } from 'lucide-react';
...
export const Wallet = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (activeAction === 'withdraw' && (user?.wallet?.totalBalance || 0) < Number(amount)) {
      setAlert({ type: 'error', message: 'Insufficient balance.' }); return;
    }
    setLoading(true);
    ...
    try {
      ...
      setAlert({ type: 'success', message: `${activeAction === 'deposit' ? 'Deposit' : 'Withdrawal'} submitted successfully.` });
      setAmount(''); setActiveAction(null); refreshUser(); fetchHistory();
    } catch (err: any) {
  ...
  const totalWithdrawn = history.filter((item: any) => item.type === 'withdraw' && (item.status === 'completed' || item.status === 'approved')).reduce((acc, curr: any) => acc + curr.amount, 0);
    ...
    <div className="px-4 md:px-8 lg:px-12 pt-6 pb-16 max-w-[1700px] mx-auto space-y-6 selection:bg-nexus-primary/20 selection:text-nexus-primary">
      ...
      <section className="space-y-4">
        <div className="flex flex-wrap items-center gap-2">
          {[
            { id: 'deposit', label: 'Deposit', icon: Download, style: 'gradient-primary text-slate-900 shadow-nexus-primary/20' },
            { id: 'withdraw', label: 'Withdraw', icon: Upload, style: 'bg-nexus-magenta text-white shadow-nexus-magenta/20' },
            { id: 'transfer', label: 'Transfer', icon: Repeat, style: 'glass border-white/10 text-slate-300' },
          ].map((action) => (
            <button key={action.id} onClick={() => setActiveAction(activeAction === (action.id as any) ? null : action.id as any)}
              className={`px-5 py-3 rounded-xl font-semibold text-xs flex items-center gap-2 transition-all hover:scale-[1.03] active:scale-95 shadow-lg ${action.style} ${activeAction === action.id ? 'ring-2 ring-white/30' : ''}`}>
            ...
            <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 16 }}
              ...
              <div className="flex justify-between items-center mb-6">
                <div className="flex items-center gap-4">
                  <div className={`w-10 h-10 rounded-xl flex items-center justify-center border ${activeAction === 'deposit' ? 'bg-nexus-primary/10 border-nexus-primary/20 text-nexus-primary' : 'bg-nexus-magenta/10 border-nexus-magenta/20 text-nexus-magenta'}`}>
                    {activeAction === 'deposit' ? <Download size={18} /> : <Upload size={18} />}
                  </div>
                  <div>
                    <h3 className="text-sm font-bold text-white capitalize">{activeAction}</h3>
                    <p className="text-[10px] text-slate-500">Enter amount and payment details</p>
                  </div>
                </div>
              ...
              <form onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                <div className="lg:col-span-7 space-y-5">
                  <div className="space-y-2">
                    <div className="flex justify-between">
                      <p className="text-[10px] text-slate-600 font-medium uppercase tracking-wider">Amount (USD)</p>
                      {activeAction === 'withdraw' && (
                        <p className="text-[10px] text-slate-500">Available: <span className="text-nexus-primary font-semibold">${user?.wallet.totalBalance.toLocaleString()}</span></p>
                      )}
                    </div>
                    <div className="bg-black/60 border border-white/5 rounded-2xl p-6 flex flex-col items-center focus-within:border-nexus-primary/30 transition-all">
                      <div className="flex items-center gap-3">
                        <span className="text-3xl font-bold text-nexus-primary opacity-20">$</span>
                        <input type="number" value={amount} onChange={(e) => setAmount(e.target.value)} placeholder="0.00" className="bg-transparent text-3xl font-bold outline-none w-full text-center text-white placeholder:text-white/5" autoFocus />
                      </div>
                      <div className="w-full h-px bg-white/5 my-4" />
                      <p className="text-[10px] text-slate-700">Minimum: $10.00</p>
                    </div>
                  </div>

                  <div className="space-y-2">
                    <p className="text-[10px] text-slate-600 font-medium uppercase tracking-wider">Payment Method</p>
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                      {['CRYPTO', 'USDT', 'BANK', 'E-WALLET'].map(m => (
                        <button key={m} type="button" onClick={() => setPaymentMethod(m)}
                          className={`p-3 rounded-xl border text-[10px] font-semibold transition-all ${paymentMethod === m ? 'border-nexus-primary bg-nexus-primary/10 text-nexus-primary' : 'border-white/5 bg-white/[0.02] text-slate-500 hover:border-white/20'}`}>
                          {m}
                        </button>
                      ))}
                    </div>
                  ...
                  {activeAction === 'withdraw' && (
                    <div className="space-y-2">
                      <p className="text-[10px] text-slate-600 font-medium uppercase tracking-wider">Destination Address</p>
                      <input type="text" placeholder="Enter your wallet address or account number" className="w-full bg-black/40 border border-white/5 rounded-xl py-3 px-4 outline-none focus:border-nexus-magenta/40 text-xs text-white placeholder:text-slate-700" />
                    </div>
                  )}
                </div>
                ...
                <div className="lg:col-span-5 flex flex-col justify-between gap-4">
                  <div className="space-y-3">
                    <div className="p-4 glass rounded-xl border border-white/5 flex items-start gap-3 hover:border-white/10 transition-colors">
                      <div className="p-2 bg-white/5 rounded-lg text-nexus-primary shrink-0">
                        <Lock size={15} />
                      </div>
                      <div>
                        <h4 className="text-xs font-semibold text-white mb-1">End-to-End Encrypted</h4>
                        <p className="text-[10px] text-slate-500 leading-relaxed">All transactions are processed through AES-512 encrypted channels with real-time audit logging.</p>
                      </div>
                    </div>
                    <div className="p-4 glass rounded-xl border border-white/5 flex items-start gap-3 hover:border-white/10 transition-colors">
                      ...
                      <div>
                        <h4 className="text-xs font-semibold text-white mb-1">Processing Time</h4>
                        <p className="text-[10px] text-slate-500 leading-relaxed">Transactions are typically processed within 1-2 hours depending on network congestion.</p>
                      </div>
                    </div>
                  </div>

                  <button type="submit" disabled={loading}
                    className={`w-full py-4 rounded-2xl flex items-center justify-center gap-3 transition-all font-semibold text-sm shadow-xl ${activeAction === 'deposit' ? 'gradient-primary text-slate-900 shadow-nexus-primary/20' : activeAction === 'withdraw' ? 'bg-nexus-magenta text-white shadow-nexus-magenta/20' : 'glass border-white/10 text-white'} hover:scale-[1.02] active:scale-95 disabled:opacity-50`}>
                    {loading ? (
                      <div className="flex items-center gap-3"><div className="w-4 h-4 border-2 border-current/30 border-t-current rounded-full animate-spin" /><span>Processing...</span></div>
                    ) : (<>Confirm {activeAction === 'deposit' ? 'Deposit' : 'Withdrawal'} <ChevronRight size={18} /></>)}
                  </button>
                </div>
              </form>
            </motion.div>
            ...
            <table className="w-full text-left border-collapse min-w-[700px]">
              <thead>
                <tr className="border-b border-white/5 bg-white/[0.01]">
                  ...
                  <th className="px-5 py-4 text-[10px] font-medium text-slate-600 uppercase tracking-wider">Amount (USD)</th>
                ...
                {filteredHistory.map((item: any) => (
                  <tr key={item._id} className="hover:bg-white/[0.02] transition-colors group">
                    ...
                    <td className="px-5 py-4">
                      <span className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-lg border text-[10px] font-medium uppercase tracking-wider ${item.type === 'deposit' || item.type === 'profit' ? 'border-nexus-primary/20 bg-nexus-primary/5 text-nexus-primary' : item.type === 'withdraw' ? 'border-nexus-magenta/20 bg-nexus-magenta/5 text-nexus-magenta' : 'border-white/10 bg-white/5 text-slate-400'}`}>
                    ...
                    <td className="px-5 py-4">
                      <span className={`text-[10px] font-medium px-3 py-1 rounded-lg border uppercase tracking-wider ${item.status === 'completed' || item.status === 'approved' ? 'bg-emerald-500/10 border-emerald-400/20 text-emerald-400' : item.status === 'pending' ? 'bg-yellow-500/10 border-yellow-500/20 text-yellow-500' : 'bg-rose-500/10 border-rose-500/20 text-rose-500'}`}>
                        {item.status}
                      </span>
                    </td>
          ...
          {filteredHistory.length === 0 && (
            <div className="p-16 flex flex-col items-center justify-center space-y-4 opacity-40">
              ...
              <div className="text-center">
                ...
                <p className="text-[10px] text-slate-500">No transactions match your current filter.</p>
              </div>
            </div>
          )}
        </div>
